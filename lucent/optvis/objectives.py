...
def penalize_boundary_complexity(shp, w=20, mask=None, C=0.5):
    """Encourage the boundaries of an image to have less variation and of color C in PyTorch.
    ...
    def inner(T):
        ...
        if mask is None:
            mask_ = torch.ones(shp, device=arr.device)
            mask_[:, :, w:-w, w:-w] = 0
        else:
            mask_ = mask

        blur = _torch_blur(arr, w=5)
        diffs = (blur - arr) ** 2
        diffs += 0.8 * (arr - C) ** 2

        return torch.sum(diffs * mask_)
...
def deepdream(layer):
    ...
    """
    def inner(T):
        # T(layer) will return the activation from the specified layer
        # The **2 represents element-wise squaring of the activation
        # torch.mean computes the mean of these squared activations
        return -torch.mean(T(layer)**2)
...
def L1(layer="input", constant=0):
    """L1 norm of layer. Generally used as a penalty."""
    def inner(T):
        return -torch.sum(torch.abs(T(layer) - constant))
...
def L2(layer="input", constant=0, epsilon=1e-6):
    """L2 norm of layer. Generally used as a penalty."""
    def inner(T):
        return -torch.sqrt(epsilon + torch.sum((T(layer) - constant) ** 2))

    return inner
    
#@wrap_objective()
#def dot_compare(layer, batch=1, cossim_pow=0): # -> lucent implementation
#  """Dot product objective, to maximize the cosine similarity between the feature representations of #a parameterized input and the target image""" 
#  def inner(T):
...
@wrap_objective() 
def dot_compare(layer, batch=1, cossim_pow=0): # -> lucid implementation
    """Dot product objective, to maximize the cosine similarity between the feature representations of a parameterized input and the target image"""
    def inner(T):
        acts1 = T(layer)[0]
        acts2 = T(layer)[batch]
        dot = torch.sum(acts1 * acts2)
        mag = torch.sqrt(torch.sum(acts2 ** 2))
        cossim = dot / (1e-6 + mag)
